...
async def process_vk_auth(message: Message, state: FSMContext):
    ...
    await message.answer(
        text="<b>Click the link to authorize:</b>",
...
@router.message(
    StateFilter(FSMVKLogin.acquire_token),
    lambda message: "access_token" in message.text,
)
async def process_vk_token(message: Message, state: FSMContext):
    access_token = re.search(r"access_token=(?P<access_token>.+?)&", message.text)[
        "access_token"
    ]
