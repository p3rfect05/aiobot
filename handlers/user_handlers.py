...
@router.message(
    F.document, lambda message: re.search(r"[.]doc|[.]docx", message.document.file_name)
)
async def process_photo(message: Message, bot: Bot):
    ...
    await bot.send_document(
        message.chat.id, document=FSInputFile(dest.replace(ext, "pdf"))
    )
...
async def save_file(
    ...
    file = list(
        filter(
            lambda x: x, [message.document, message.video, message.audio, message.photo]
        )
    )[0]
    ...
    async with session_maker.begin() as session:
        new_file = StorageModel(
            file_id=file_id,
            file_type=file_type,  # noqa
            file_name=file_name,
...
async def show_folder(callback: CallbackQuery, session_maker: async_sessionmaker):
    ...
    async with session_maker.begin() as session:
        ...
        buttons = {
            f"download:{file.id}:{file_type}": file.file_name for file in display_files
        }
        buttons.update({"choose_delete_file": "Delete file"})
        precise_pages = len(files) // FILES_PER_PAGE
        total_pages = (
            precise_pages + 1 if len(files) % FILES_PER_PAGE else precise_pages
        )
        if not total_pages:
            total_pages = 1
        forward_backward_buttons: list[InlineKeyboardButton] = []
        if page > 1:
            forward_backward_buttons.append(
                InlineKeyboardButton(
                    text="<<", callback_data=f"storage:{file_type}:{page - 1}"
                )
            )
        forward_backward_buttons.append(
            InlineKeyboardButton(
                text=f"{page}/{total_pages}", callback_data=f"{page}/{total_pages}"
            )
        )
...
async def delete_file(callback: CallbackQuery, session_maker: async_sessionmaker):
    ...
    try:
        ...
        await callback.message.answer(
            "Nothing to delete! (must be a bug then! report pls.)"
        )
    await callback.answer()
    await callback.message.delete()
...
async def process_time_selection(message: Message, state: FSMContext):
    ...
    try:
        ...
        if check_reminder_time(hours, minutes, date):
            await message.answer(
                f"Now set the description for the reminder (not exceeding 100 symbols)"
            )
        ...
        await message.answer(
            "Invalid format for the time (or you set the reminder in the past)!"
        )


@router.message(
    F.text.as_("desc"), F.text.len() < 100, StateFilter(FSMReminder.set_description)
)
async def process_desc_setting(
    message: Message,
    state: FSMContext,
    session_maker: async_sessionmaker,
    desc: str,
    ...
    async with session_maker.begin() as session:
        new_reminder = ReminderModel(
            reminder_type="date",
            reminder_desc=desc,  # noqa
            user_fk=message.from_user.id,
            time_triggers=date_with_time,
        )  # noqa
        session.add(new_reminder)
    apscheduler.add_job(
        send_reminder,
        trigger="date",
        run_date=date_with_time,
...
async def process_invalid_desc_setting(message: Message):
    await message.answer(
        "Invalid description: it must be a string not exceeding 100 symbols!"
    )


@router.message(F.text, Command(commands="to_rus"))
async def translate_text(
    message: Message, command: CommandObject, en_to_ru_translator: GoogleTranslator
):
    text = command.args
    if not text:
...
async def translate_text(
    message: Message, command: CommandObject, ru_to_en_translator: GoogleTranslator
):
    text = command.args
    if not text:
        await message.answer("Text to translate should not be <b>empty</b>.")
