...
def user_groups(user_id):
    ...
    try:
        response = requests.post(base + 'users.getSubscriptions?', params=params).json()['response']['users']['items']
...
def whole_group_analysis(group_id, if_user_in):
    ...
    checked = 0
    members_number = 0
    params = {'access_token': access_token, 'v': '5.313', 'group_id': group_id, 'offset' : 0}
    while True:
        try:
            response = requests.post(base + 'groups.getMembers?', params=params)
        except:
            return f'Something went wrong with group {group_id} and user {initial_user_id}'
        time.sleep(0.5)
...
def get_id(user_id):
    try:
        ...
        response = requests.post(base + 'users.get?', params=params).json()['response'][0]['id']
    except:
