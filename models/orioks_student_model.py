...
class OrioksGroupsModel(BaseModel):
    ...
    students: Mapped[list["OrioksStudentsModel"]] = relationship(
        back_populates="group", lazy="selectin", uselist=True
    )
...
class OrioksStudentsModel(BaseModel):
    ...
    group: Mapped["OrioksGroupsModel"] = relationship(
        back_populates="students", lazy="selectin", uselist=False
    )
    group_fk = mapped_column(ForeignKey("orioks_groups.group_id"))
    user: Mapped["UserModel"] = relationship(
        back_populates="orioks_student", uselist=False, lazy="selectin"
    )
